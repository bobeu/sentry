 ...
 */
export const EMPLOYMENT_AGREEMENT_VERSION = "2026-07-24";

export const EMPLOYMENT_AGREEMENT_TITLE =
  "Sentry Employment Agreement (AI Telegram Employee)";
...
export function employmentAgreementPromptBrief() {
  return [
    ...
    "- Parties: Employer (human account holder) and Sentry (AI Telegram employee operated via the Sentry service on Celo).",
    "- Work: moderate spam, answer FAQs/KB, welcome members, summaries/reports, optional engagement (polls/games/points/rewards), secretary/admin tools in enabled groups only.",
    "- Pay: prepaid employment wallet on Celo; bill per completed action at published rates; no work when balance exhausted. Optional member cash rewards use a separate RewardAccount when Employer funds it.",
    "- Privacy: Employer group content used only to perform hired duties; not sold; Employer controls FAQs/KB/playbook/engagement settings.",
...
────────────────────────────────
1.1 Sentry is an artificial-intelligence Telegram employee for communities—not a human employee, contractor in the traditional labor-law sense, attorney, compliance officer, or fiduciary.
...
1.3 Accepting this Agreement authorizes Sentry to perform the duties described below in groups Employer enables, using FAQs, knowledge bases, playbooks, engagement settings, and other controls Employer configures.
...
2.2 Engagement duties (when Employer enables them): host polls, trivia/quizzes, fun/comics, and social campaigns; award points; optionally pay cash rewards from a separate RewardAccount Employer funds (not the employment wallet). Members may check status via /mystatus or /points.
2.3 Sentry works only in Telegram chats/groups Employer connects and enables. Sentry will not claim capabilities it is not configured or funded to perform.
2.4 Employer may set personas, playbooks, escalation rules, report cadence, engagement guidelines, and per-group settings. Corrections Employer teaches may update living playbooks for that employment.
2.5 Sentry may escalate ambiguous or high-stakes drafts to Employer when escalation is enabled.
...
3.2 Each completed billable action creates a charge at the rates published in the Sentry Pricing page / dashboard (subject to demo or promotional adjustments if clearly labeled). Examples include mention replies, FAQ answers, spam moderation, summaries, engagement activities, points awards, reward payouts, and related agent actions.
...
3.5 Withdrawals from the employment wallet (if enabled) go only to Employer’s confirmed withdrawal destination. Employer must protect their wallet keys; Sentry cannot recover lost keys.
3.6 Optional member cash rewards are paid from a separate RewardAccount controlled via Sentry’s operator key. Employer funds that account; members withdraw pending reward balances by providing a destination address to Sentry. Reward custody does not modify the employment wallet contracts.
3.7 Gas/network fees on Celo for Employer-initiated transfers are Employer’s responsibility.
