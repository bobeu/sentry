...
export const SIMULATED_CONVERSATIONS: Record<string, { trigger: string; label: string; messages: ChatMessage[] }> = {
  faq: {
...
export function HeroSlideshow() {
  ...
  return (
    <div className="relative flex flex-col items-center justify-center animate-rise-delay-2 h-[400px] w-full">
...
export function FeatureCarousel() {
  ...
  const featureDetails = [
    { title: "AI Operations Dashboard", desc: "Track active sessions, response throughput, and verify overall performance indicators.", image: "/dashboard.png" },
    { title: "Pending Escalation Approvals", desc: "Review drafted messages for high-stakes triggers prior to sending.", image: "/escalation_dashboard.png" },
    { title: "Shift Handover Logs", desc: "Structured briefs delivered directly to the team summarizing events.", image: "/handover.png" },
    { title: "Adaptive Playbooks", desc: "Teach policies in plain text. Rules are instantly contextualized by the AI.", image: "/sentry_learns_your_rules.png" },
    { title: "Dedicated Persona Roles", desc: "Customize tones, support roles, and instructions differently per group chat.", image: "/many_roles.png" },
    { title: "Engagement & Rewards", desc: "Polls, trivia, and learn-and-earn with points — optional cash payouts from a separate RewardAccount.", image: "/sentry_real_work.png" }
  ];
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <div className="bg-white border border-primary/10 rounded-2xl p-5 shadow-sm min-h-[90px] flex flex-col justify-center">
        <AnimatePresence mode="wait">
          <motion.div
            key={`feat-${activeFeatureSlide}`}
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            className="space-y-1 text-center sm:text-left"
          >
            <h3 className="font-extrabold text-lg text-text-dark">
              {featureDetails[activeFeatureSlide].title}
            </h3>
            <p className="text-xs text-muted font-semibold leading-relaxed">
              {featureDetails[activeFeatureSlide].desc}
            </p>
          </motion.div>
        </AnimatePresence>
      </div>
      ...
      <div className="relative w-full aspect-[16/10] sm:aspect-[16/9] rounded-2xl border border-primary/10 overflow-hidden shadow-xl bg-white/40 backdrop-blur-xl">
        <AnimatePresence mode="wait">
          <motion.div
            key={`featslide-${activeFeatureSlide}`}
          ...
          >
            <Image
              src={featureDetails[activeFeatureSlide].image}
              alt={featureDetails[activeFeatureSlide].title}
              fill
